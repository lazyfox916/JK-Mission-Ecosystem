      ...
      <div className="flex items-center justify-center min-h-[60vh]">
        <Card className="max-w-md w-full text-center" padding="lg">
          ...
          <p className="text-sm text-text-muted mb-1">
            Patient <strong className="text-text-primary">{form.name}</strong>{" "}
            has been registered.
          </p>
          <p className="text-xs text-text-muted mb-6">
            AI analysis has been queued. You&#39;ll be notified when results are
            ready.
          </p>
      ...
      <div className="flex items-center gap-3">
        {[
          ...
          <div key={n} className="flex items-center gap-2">
            ...
            <span
              className={`text-sm font-medium ${step >= n ? "text-text-primary" : "text-text-muted"}`}
            >
              {label}
            </span>
            {n < 2 && (
              <div
                className={`h-px w-12 transition-all duration-200 ${step > n ? "bg-neon-purple" : "bg-border"}`}
              />
            )}
          </div>
        ))}
        ...
        <div className="space-y-4">
          ...
          <Card padding="lg">
            ...
            <p className="text-xs text-text-muted mb-5">
              Upload scans, reports, X-rays, or any relevant medical documents.
              AI will analyze them automatically.
            </p>
            <FileUploader accept="image/*,video/*,.pdf,.doc,.docx" multiple />
          </Card>
